...
if 'master_data' in st.session_state:
    ...
    try:
        ...
        st.warning("⚠️ No active user upload detected. Displaying local 'capstone_dataset_2years.csv' sample file as fallback.")
    except FileNotFoundError:
        st.error("❌ Please upload a CSV file on the Home Page (app.py) first to look at dashboard metrics.")
        st.stop()
...
st.markdown("### 📊 Operational & Financial Trend Inquiries")
...
with filter_wrapper_col:
    ...
    selected_year = st.selectbox("📅 Select Timeline Horizon", options=year_options)
...
with table_col2:
    st.markdown("#### 🔴 Top 5 Highest Spoilage Products")
    st.dataframe(
...
st.markdown("---")
st.markdown("### 📋 Historical Inventory Ledger & Performance Breakdown")
...
with filter_col1:
    year_options = ["All Years"] + sorted(list(df['Year'].unique().astype(str)))
    selected_table_year = st.selectbox("📅 Table Year Horizon", options=year_options, key="table_year_picker")
...
with filter_col2:
    ...
    selected_table_month = st.selectbox("📆 Table Month Filter", options=month_options, key="table_month_picker")
...
if not ledger_df.empty:
    ...
    st.info("ℹ️ No operational records match this specific Year/Month time filter configuration.")
